...
var betCursor=db2.bfraces.find({rpraceid:{$exists:true}});
        ...
        if(race && (race.racetype==nconf.get('racetype'))){
            ...
            db.spbets.findOne({"rpraceid":rpraceid},function(err,spbet){
                if(spbet){
                  ...
                  console.log("node "+ nconf.get('scrapepath')+"generateBFPredictionCalls --conf=" + nconf.get('confdir') + "spbetconf.json --raceid=" + rpraceid);
                  doProcessOneBet();
                }
            });
