...
var collections=["races","horses","cards"];
  ...
  request({url:raceUrl},function(error,response,body){
    ...
    try{
      ...
      //  }
       ...
       asyncCalls++;
       db.cards.findOne({rpraceid:raceDocument.rpraceid},function(err,card){
          asyncCalls--;
          ...
          if(err){
            ...
            //logger.info("Card is: " + JSON.stringify(card));
            if(card == null){
              asyncCalls++;
       ...
       //At this point, tell the server to schedule a task(s) to monitor this race
...
function processRaceResultData(raceid){
